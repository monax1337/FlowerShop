...
const readBouquetsFromFile = (language) => {
    try {
        const selectedLanguage = language || initialLanguage;
        const data = fs.readFileSync(path.join(__dirname, `BouquetsData_${selectedLanguage}.json`));
        return JSON.parse(data);
    } catch (error) {
        console.error('Ошибка при чтении файла: ', error);
        return [];
...
const writeFlowersToFile = (bouquets, language) => {
    try {
        fs.writeFileSync(path.join(__dirname, `FlowersData_${language}.json`), JSON.stringify(bouquets, null, 2));
...
const writeBouquetsToFile = (bouquets, language) => {
    try {
        fs.writeFileSync(path.join(__dirname, `BouquetsData_${language}.json`), JSON.stringify(bouquets, null, 2));
        console.log('Изменения сохранены в файле.');
    } catch (error) {
        console.error('Ошибка при записи в файл: ', error);
    }
...
app.post('/bouquets', (req, res) => {
    const {newBouquet, language} = req.body;
    let bouquets = readBouquetsFromFile(language);
    bouquets.push(newBouquet);
    writeBouquetsToFile(bouquets, language);
    res.send('Bouquet added successfully.');
});
...
app.put('/bouquets', (req, res) => {
    const {oldBouquet, newBouquet, language} = req.body;
    const bouquets = readBouquetsFromFile(language);
    ...
    writeBouquetsToFile(updatedBouquets, language);
    res.send('Bouquet updated successfully.');
});
...
app.delete('/bouquets', (req, res) => {
    const {bouquetId, language} = req.body;
    const bouquets = readBouquetsFromFile(language);

    const updatedBouquets = bouquets.filter(bouquet => bouquet.id !== bouquetId);
...
app.get('/bouquets', (req, res) => {
    const language = req.query.language;
    const flowers = readBouquetsFromFile(language);
    res.send(flowers);
});
